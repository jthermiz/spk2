...
if ~isempty(ops.chanMap)
    if ischar(ops.chanMap)
        ...
        try
            ...
            chanMapConn = 1+chanNums(connected>1e-6); err for some reason?
            %chanMapConn = 1+chanMap(connected>1e-6);
...
while 1
    ...
    if nsampcurr<NTbuff
        buff(:, nsampcurr+1:NTbuff) = repmat(buff(:,nsampcurr), 1, NTbuff-nsampcurr); %pads end of DATA
    end
    if ops.GPU
